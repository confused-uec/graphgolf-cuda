...
int main(int argc, char* argv[]){
    ...
    p.load(ist);
    if(logging){
        logfs<<"Nx: "<<p.Nx<<" Ny: "<<p.Ny<<" Nz: "<<p.Nz<<" Mx: "<<p.Mx<<" My: "<<p.My<<" Mz:  "<<p.Mz<<std::endl;
    }

    int device=0;
    if(vm.count("device")) device=vm["device"].as<int>();
    graphgolf::cudaASPLbox cu(p.Nx,p.Ny,p.Nz,p.Mx,p.My,p.Mz,p.degree,device);
    //graphgolf::cpuASPLbox<65536> cu;
